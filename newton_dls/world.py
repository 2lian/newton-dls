...
class Example:
    def __init__(self, viewer, args):
        ...
        builder.add_shape_box(
            body=-1,  # attached to world, not to a rigid body
            xform=wp.transform((0, 0, -0.500), wp.quat_identity()),
            hx=5,
            hy=5,
            hz=0.5,
            label="ground_box",
        ...
        # add sand particles
        density = 2500.0
        particle_lo = np.array([-0.5, -0.6, 0.0])  # emission lower bound
        particle_hi = np.array([0.5, 2.5, 0.4])  # emission upper bound
        ...
        # self.model.mpm.yield_pressure.fill_(40)
